...
{	int j, h = now._nr_pr;
	...
	case 1:	/* Q */
		...
		((P1 *)pptr(h))->q_count = 0;
#ifdef VAR_RANGES
		logval("Q:q_count", ((P1 *)pptr(h))->q_count);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* P */
		...
		logval("P:p_count", ((P0 *)pptr(h))->p_count);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
#ifdef VAR_RANGES
		logval("n", now.n);
		logval("p_complete", now.p_complete);
#endif
}
...
{	/* int i; */
	...
	printf("	byte   q_count:	%d\n", ((P1 *)pptr(pid))->q_count);
		break;
	case 0:
		printf("local vars proc %d (P):\n", pid);
	printf("	byte   temp:	%d\n", ((P0 *)pptr(pid))->temp);
	printf("	byte   p_count:	%d\n", ((P0 *)pptr(pid))->p_count);
		break;
	}
}
void
